...
function prolongation_matrix_p4(N)
    # SBP preserving
    # N = 2^level + 1
...
function restriction_matrix_p4(N)
    # SBP preserving
    ...
    # odata[end,end-1] = 1/2
    # odata[end,end] = 1/2
    odata[1,1:10] = [1/2, 429/544,0,-103/544,-5/34,-27/544,1/17,23/544,0,-1/272]
    odata[2,1:10] = [0, 279/944, 43/118, 549/1888,5/59,15/1888,-3/118,-37/1888,0,3/1888]
    odata[end,end-9:end] = reverse([1/2, 429/544,0,-103/544,-5/34,-27/544,1/17,23/544,0,-1/272])
    odata[end-1,end-9:end] = reverse([0, 279/944, 43/118, 549/1888,5/59,15/1888,-3/118,-37/1888,0,3/1888])
    for i in 3:div(N+1,2)-2
        odata[i,2*i-4] = -1/32
        odata[i,2*i-3] = 0
        odata[i,2*i-2] = 9/32
        odata[i,2*i-1] = 1/2
...
function prolongation_2d_p4(N)
    prolongation_1d = prolongation_matrix_p4(N)
    prolongation_2d = kron(prolongation_1d,prolongation_1d)
    return prolongation_2d
end

function restriction_2d_p4(N)
    restriction_1d = restriction_matrix_p4(N)
