...
function matrix_free_N_D2_kernel_v2(idata,odata,coef_D,Nx,Ny,hx,hy,::Val{TILE_DIM1},::Val{TILE_DIM2}) where {TILE_DIM1, TILE_DIM2}
    tidx = threadIdx().x
    tidy = threadIdx().y
    ...
    if 1 <= i <= 4 && 5 <= j <= Ny-4
        ...
        + coef_D.bd[i,1] * idata[1,j] + coef_D.bd[i,2] * idata[2,j] + coef_D.bd[i,3] * idata[3,j] + coef_D.bd[i,4]*idata[4,j] + coef_D.bd[i,5]*idata[5,j] + coef_D.bd[i,6] * idata[6,j]) / (coef_D.bhinv[i]) # calculation for the left upper corner
    end
    nothing
end
...
function matrix_free_N_D2_GPU_v2(idata,odata,coef_D,Nx,Ny,hx,hy) 
    TILE_DIM_1 = 16
    TILE_DIM_2 = 16
    griddim = (div(Nx+TILE_DIM_1+1,TILE_DIM_1),div(Ny+TILE_DIM_2-1,TILE_DIM_2))
    blockdim = (TILE_DIM_1,TILE_DIM_2)
    @cuda threads=blockdim blocks=griddim matrix_free_N_D2_kernel_v2(idata,odata,coef_D,Nx,Ny,hx,hy,Val(TILE_DIM_1),Val(TILE_DIM_2))
end
